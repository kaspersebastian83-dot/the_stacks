...
assert.match(markdown,/- Copies with a recorded location: 3\n- Copies needing a location: 1/);
...
assert.ok(markdown.includes('Living Room → Main Bookcase → Shelf 3 → Box 2 → \\#4'));
assert.match(markdown,/Office → Bookcase 1 → Shelf 5/);
assert.match(markdown,/Copy: Not assigned/);
assert.match(markdown,/Copy: Study/,'Partial recorded location is kept as recorded');
