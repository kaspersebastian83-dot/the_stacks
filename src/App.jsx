...
function MoveCopiesDialog({books,copyIds,onConfirm,onClose}){
  ...
  const submit=()=>{if(!hasMoveDestination(destination)){setError('Choose at least a Room, Bookcase, Shelf, or Box.');return;}const result=onConfirm(copyIds,destination);if(result?.error){setError(result.error);return;}onClose();};
