...
function BooksLibraryView({books,collections,filters,setFilters,savedViews,setSavedViews,onEdit,onQuickEdit,onExport,onBulkUpdate,onBulkDelete,onBulkRetryMetadata,browseTabs,visible,effectiveFilters,debouncedQuery}){
  const tags=useMemo(()=>uniq(books.flatMap(b=>b.tags||[])).sort(),[books]);const rooms=useMemo(()=>uniq(books.map(b=>normalizeLocation(b.location).room)).sort(),[books]);const bookcases=useMemo(()=>uniq(books.filter(b=>!filters.room||normalizeLocation(b.location).room===filters.room).map(b=>normalizeLocation(b.location).bookcase)).sort(),[books,filters.room]);const physicalShelves=useMemo(()=>uniq(books.filter(b=>(!filters.room||normalizeLocation(b.location).room===filters.room)&&(!filters.bookcase||normalizeLocation(b.location).bookcase===filters.bookcase)).map(b=>normalizeLocation(b.location).shelf)).sort(),[books,filters.room,filters.bookcase]);const boxes=useMemo(()=>uniq(books.filter(b=>(!filters.room||normalizeLocation(b.location).room===filters.room)&&(!filters.bookcase||normalizeLocation(b.location).bookcase===filters.bookcase)&&(!filters.physicalShelf||normalizeLocation(b.location).shelf===filters.physicalShelf)).map(b=>normalizeLocation(b.location).box)).sort(),[books,filters.room,filters.bookcase,filters.physicalShelf]);const [visibleLimit,setVisibleLimit]=useState(200);const [filtersOpen,setFiltersOpen]=useState(false);const [viewsOpen,setViewsOpen]=useState(false);useEffect(()=>setVisibleLimit(200),[effectiveFilters,books.length]);const rowsToShow=visible.slice(0,visibleLimit);const [selected,setSelected]=useState([]);const [bulkStatus,setBulkStatus]=useState('');const [bulkCollection,setBulkCollection]=useState('');const [bulkTag,setBulkTag]=useState('');const [bulkRoom,setBulkRoom]=useState('');const [bulkBookcase,setBulkBookcase]=useState('');const [bulkShelf,setBulkShelf]=useState('');const selectedBooks=visible.filter(b=>selected.includes(b.id));
...
function HomeCoverCard({book,reading=false,onEdit}){
  return <button className={reading?'home-reading-card':'home-cover-card'} onClick={()=>onEdit?.(book)} aria-label={'Open '+(book.title||'book')}><div className="cover">{book.cover?<img loading="lazy" src={book.cover} alt=""/>:<Icon name="book" size={reading?32:26}/>}<ReadBadge book={book}/></div><div className={reading?'home-reading-copy':''}><b className="truncate">{book.title||'Untitled book'}</b><span className="truncate">{book.authors||'Unknown author'}</span>{reading&&book.startedAt&&<small>Started {book.startedAt}</small>}</div></button>;
}

function DashboardView({books,collections,savedViews,setActiveView,setActiveCollection,setFilters,onExportJSON,onImportJSON,settings,onAddManual,onLoadDemo,onOpenAddBooks,onOpenSort,onOpenOrganize,onEdit}){
