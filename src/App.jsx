...
function CameraBarcodeScanner({active,onDetected,onStop,minimal=false,onInvalidDetected,onUnavailable,mode='add'}){
  ...
  useEffect(()=>{
    ...
    const handleFrame=value=>{
      ...
      setState({status:valid?'found':'scanning',message:valid?`Scanned ${toISBN13(frame.code)}. ${mode==='find'?'Checking…':'Adding…'}`:'That does not look like a book barcode.'});
      if(!valid)onInvalidDetected?.(frame.code);
      Promise.resolve(onDetected?.(frame.code)).catch(()=>{const message=mode==='find'?'The book could not be checked. Enter the ISBN below.':'The book could not be added. Use the Scan Desk instead.';setState({status:'error',message});onUnavailable?.(message);}).finally(()=>{if(!cancelled)setState({status:'scanning',message:'Point the camera at the ISBN barcode.'});});
    };
  ...
  const label=state.status==='scanning'?'Scanning':state.status==='found'?(mode==='find'?'Checking book':'Adding book'):state.status==='error'?'Camera needs help':'Starting camera';
...
function EasyScan({collections,settings,onScan,onUndoLast,onDone,onUseScanDesk}){
  ...
  if(!session){const location=formatLocationSummary(setup.location);return <div className="easy-scan"><div className="easy-setup"><div className="easy-setup-card"><h1>Add books</h1><p>Choose where this group of books belongs, then start scanning.</p><div className="easy-setup-fields"><label><div className="label">Collection</div><select className="select" value={setup.collection} onChange={e=>setSetup(current=>({...current,collection:e.target.value}))}><option value="">Unshelved</option>{collections.map(collection=><option key={collection} value={collection}>{collection}</option>)}</select></label><label><div className="label">Reading status</div><select className="select" value={setup.status} onChange={e=>setSetup(current=>({...current,status:e.target.value}))}>{STATUS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label><div className="label">Room</div><input className="field" value={setup.location.room} onChange={e=>updateLocation('room',e.target.value)}/></label><label><div className="label">Bookcase</div><input className="field" value={setup.location.bookcase} onChange={e=>updateLocation('bookcase',e.target.value)}/></label><label><div className="label">Shelf / location</div><input className="field" value={setup.location.shelf} onChange={e=>updateLocation('shelf',e.target.value)}/></label></div><div className="easy-setup-summary"><b>Books will be added to:</b><br/>Collection: {setup.collection||'Unshelved'}<br/>Location: {location}<br/>Status: {statusLabel(setup.status)}</div><div className="label">How would you like to scan?</div><div className="easy-method-choice"><button className={setup.method==='camera'?'selected':''} onClick={()=>setSetup(current=>({...current,method:'camera'}))}><b>Camera</b><small>Use the device camera.</small></button><button className={setup.method==='scanner'?'selected':''} onClick={()=>setSetup(current=>({...current,method:'scanner'}))}><b>Barcode scanner</b><small>Use a USB or Bluetooth scanner.</small></button></div><button className="btn easy-setup-start" onClick={begin}>Start scanning</button></div><button className="ghost" onClick={()=>onDone?.([])}>Back</button></div></div>}
...
function ScanDesk({books,collections,onScan,onAddManual,onEdit,onDeleteIds,onSortSession,settings,setSettings,focusBatch=false,onBatchFocused}){
  ...
  const submit=()=>{const v=val.trim();if(v){runScan(v,'manual');setVal('');}};
  ...
  const updateLocation=(key,value)=>setSettings?.(s=>normalizeSettings({...s,defaultLocation:{...normalizeLocation(s?.defaultLocation),[key]:value}}));
  return <div className="workspace grid2"><section className="panel panel-pad"><div className="overline">Scan Desk</div><h1 className="title">Batch book intake</h1><p className="subtitle">Scan one ISBN, run the camera continuously, or paste a whole list of ISBNs. New scans use the selected collection and default location below.</p>
    ...
    <div className="panel soft-panel" style={{marginTop:12,padding:12}}><div className="label">Default location for new scans</div><div className="location-grid"><input className="field" placeholder="Room" value={defaultLoc.room} onChange={e=>updateLocation('room',e.target.value)}/><input className="field" placeholder="Bookcase" value={defaultLoc.bookcase} onChange={e=>updateLocation('bookcase',e.target.value)}/><input className="field" placeholder="Shelf" value={defaultLoc.shelf} onChange={e=>updateLocation('shelf',e.target.value)}/><input className="field" placeholder="Box" value={defaultLoc.box} onChange={e=>updateLocation('box',e.target.value)}/></div><div className="small" style={{marginTop:6}}>Tip: set a location before scanning a physical shelf, then all new books receive that location automatically.</div></div>
...
function buildBookcaseNavigationIndex(catalog){
  const rooms=new Map(),missing=[],partial=[],seen=new Set();let located=0,bookcaseCount=0,shelfCount=0;
  for(const copy of catalog?.copies||[]){
    ...
    if(!roomNode.bookcases.has(bookcase)){roomNode.bookcases.set(bookcase,{label:bookcase,copies:[],shelves:new Map()});bookcaseCount++;}
    const bookcaseNode=roomNode.bookcases.get(bookcase);bookcaseNode.copies.push(copy);
  ...
  return {rooms:sorted([...rooms.values()].map(room=>({...room,bookcases:sorted([...room.bookcases.values()].map(bookcase=>({...bookcase,shelves:sorted(bookcase.shelves.values())})))}))),missing,partial,counts:{copies:seen.size,located,missing:missing.length,partial:partial.length,rooms:rooms.size,bookcases:bookcaseCount,shelves:shelfCount}};
}
...
function BookcasesLibraryView({catalog,books,onEdit,browseTabs}){
  const index=useMemo(()=>buildBookcaseNavigationIndex(catalog),[catalog]);
  const [path,setPath]=useState({room:'',bookcase:'',shelf:'',special:''});
  ...
  const room=index.rooms.find(item=>item.label===path.room);
  const bookcase=room?.bookcases.find(item=>item.label===path.bookcase);
  const shelf=bookcase?.shelves.find(item=>item.label===path.shelf);
  const invalidPath=Boolean(path.room&&!room||path.bookcase&&!bookcase||path.shelf&&!shelf||path.special==='missing'&&!index.missing.length||path.special==='partial'&&!index.partial.length);
  useEffect(()=>{if(invalidPath)setPath({room:'',bookcase:'',shelf:'',special:''});},[invalidPath]);
  const active=invalidPath?{room:'',bookcase:'',shelf:'',special:''}:path;
  ...
  const cards=active.room?(active.bookcase?bookcase?.shelves||[]:room?.bookcases||[]):index.rooms;
  const cardKind=active.room?(active.bookcase?'Shelf':'Bookcase'):'Room';
  const heading=active.special==='missing'?'Needs a location':active.special==='partial'?'Incomplete location':active.shelf?physicalLocationParts({shelf:active.shelf}).primary:active.bookcase?active.bookcase:active.room||'Bookcases';
  ...
  const openCard=label=>setPath(current=>current.room?current.bookcase?{...current,shelf:label}:{...current,bookcase:label}:{room:label,bookcase:'',shelf:'',special:''});
  return <div className="workspace library-shell bookcases-library">
    <div className="library-topbar"><div><div className="overline">Library · Physical locations</div><h1 className="title">{heading}</h1></div><div className="small">{countLabel(index.counts.copies)} in the library</div></div>
    {browseTabs}
    <nav className="bookcase-breadcrumbs" aria-label="Physical location breadcrumbs">
      <button onClick={()=>setPath({room:'',bookcase:'',shelf:'',special:''})} aria-current={!active.room&&!active.special?'page':undefined}>All Bookcases</button>
      {active.room&&<><span aria-hidden="true">›</span><button onClick={()=>setPath({room:active.room,bookcase:'',shelf:'',special:''})} aria-current={!active.bookcase?'page':undefined}>{active.room}</button></>}
      {active.bookcase&&<><span aria-hidden="true">›</span><button onClick={()=>setPath({room:active.room,bookcase:active.bookcase,shelf:'',special:''})} aria-current={!active.shelf?'page':undefined}>{active.bookcase}</button></>}
    ...
    {!active.room&&!active.special&&<div className="bookcase-stats" aria-label="Physical library overview">
    ...
    {inventory?<section className="bookcase-inventory" aria-label={heading+' inventory'}>
      <div className="bookcase-section-head"><div><div className="overline">{active.special?'Physical copies needing attention':'Shelf inventory'}</div><h2>{heading}</h2><p>{countLabel(sortedInventory.length)}{active.shelf?' assigned to this exact shelf.':' in this group.'}</p></div></div>
      <div className="bookcase-copy-list" role="list">{sortedInventory.map(copy=>{const book=booksByCopyId.get(copy.id),peerIds=editionCopies.get(copy.editionId)||[],number=peerIds.indexOf(copy.id)+1;return <div className="bookcase-copy-row" role="listitem" key={copy.id}>
        ...
        <button className="ghost bookcase-copy-open" onClick={()=>book&&onEdit(book)} disabled={!book} aria-label={'Open '+(book?.title||'copy')+' details'}>Open / edit</button>
      </div>})}</div>
    </section>:<>
...
function LibraryView(props){
  ...
  if(browseMode==='bookcases')return <BookcasesLibraryView catalog={props.catalog} books={books} onEdit={onEdit} browseTabs={tabs}/>;
...
function DashboardView({books,collections,savedViews,setActiveView,setActiveCollection,setFilters,onExportJSON,onImportJSON,settings,onAddManual,onLoadDemo,onOpenAddBooks,onOpenFind,onOpenSort,onOpenOrganize,onEdit}){
...
function EditModal({book,collections,onSave,onDelete,onClose,onPreviewMetadata,editionCopyCount=1}){
  ...
  return <div className="modal-back" onClick={onClose} role="presentation"><div ref={modalRef} className="modal book-detail-modal" role="dialog" aria-modal="true" aria-label="Edit book details" tabIndex="-1" onClick={event=>event.stopPropagation()}>
    ...
    </div><div className="modal-head modal-foot book-detail-footer"><span className="small">Changes apply when you save.</span><div><button className="ghost" onClick={onClose}>Cancel</button><button className="btn" onClick={()=>onSave(d)}><Icon name="save" size={13}/> Save changes</button></div></div>
  </div></div>;
}
...
function SettingsView({books,collections,settings,setSettings,onExportJSON,onImportJSON,onRestoreSnapshot,onRepair,onRebuildCollections,onClearSavedViews,onDownloadCSV,onPurgeEmptyCollections,onLoadDemo,onClearDemo,onOpenView,storagePanel}){
  ...
  return <><div className="workspace quiet-settings"><input ref={jsonRef} type="file" accept=".json,application/json" style={{display:'none'}} onChange={e=>{const f=e.target.files?.[0];if(f)onImportJSON(f,jsonIntent);e.target.value=''}}/><div className="overline">Settings</div><h1 className="title">Settings</h1><div className="quiet-settings-menu"><button onClick={()=>toggle('appearance')}>Appearance <span>›</span></button><button onClick={()=>toggle('defaults')}>Book defaults <span>›</span></button><button onClick={()=>toggle('backup')}>Backup & restore <span>›</span></button><button onClick={()=>toggle('library-data')}>Library data <span>›</span></button><button onClick={()=>toggle('import')}>Import & export <span>›</span></button><button onClick={()=>toggle('advanced')}>Advanced <span>›</span></button></div>
    ...
    {section==='import'&&<section className="quiet-settings-section"><div className="label">Advanced import & export</div><p className="small">JSON backup is the complete recovery format. CSV is a spreadsheet export only.</p><div className="row-actions"><button className="ghost" onClick={()=>chooseJson('merge')}>Merge a JSON file</button><button className="ghost" onClick={()=>onDownloadCSV(books,'all')}>Export CSV</button><button className="ghost" onClick={onExportJSON}>Save backup file</button></div></section>}
