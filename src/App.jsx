...
import {applyShelfAuditScan,auditSummary,createShelfAuditState,finishShelfAudit,markShelfAuditPresent,undoShelfAuditScan} from "./core/shelf-audit.mjs";
...
const CHANGELOG=[
  {version:'3.10.8',date:'2026-09-24',items:['Added Shelf Audit Mode to compare scans with a frozen physical shelf inventory, identify missing and misplaced books, and offer explicit exact-Copy corrections.']},
  {version:'3.10.7',date:'2026-09-24',items:['Added one-click bulk retry for unidentified ISBNs, with sequential provider lookups, duplicate ISBN suppression, progress feedback, and a final recovery summary.']},
  {version:'3.10.6',date:'2026-09-24',items:['Improved unresolved ISBN recovery with direct Open Library edition lookup and a conservative direct Google Books fallback, while preserving exact-match safety.']},
  {version:'3.10.5',date:'2026-09-24',items:['Added one-click metadata retry for unresolved ISBN books, with safe Edition identification and automatic resolution of matching unidentified physical copies.']},
...
function hasOnlyRequestedSourceIsbns(candidate,isbn){
  const reported=providerReportedIsbns(candidate),requested=toISBN13(normalizeISBN(isbn));
  return Boolean(requested&&reported.length&&reported.every(value=>value===requested));
}
function openLibraryEditionToMeta(edition){
  if(!edition||typeof edition!=='object')return null;
  const sourceIsbns=[...(Array.isArray(edition.isbn_13)?edition.isbn_13:[]),...(Array.isArray(edition.isbn_10)?edition.isbn_10:[])];
  const isbns=uniq(sourceIsbns.map(normalizeISBN).filter(value=>value&&isValidISBN(value)).map(toISBN13));
  const format=String(edition.physical_format||'').trim();
  const languageKey=String(edition.languages?.[0]?.key||'').split('/').pop().toLowerCase();
  const coverId=Array.isArray(edition.covers)?edition.covers.find(id=>Number.isInteger(Number(id))&&Number(id)>0):null;
  return normalizeMetadataCandidate({title:edition.title||'',authors:'',year:String(edition.publish_date||'').match(/\b\d{4}\b/)?.[0]||'',publisher:(edition.publishers||[]).map(item=>typeof item==='string'?item:item?.name||'').filter(Boolean).join(', '),pages:edition.number_of_pages||'',language:LANGUAGE_LABELS[languageKey]||languageKey,edition:edition.edition_name||'',format,translators:[],editors:[],tags:(edition.subjects||[]).filter(item=>typeof item==='string').slice(0,5),cover:coverId?`https://covers.openlibrary.org/b/id/${coverId}-L.jpg`:'',isbn:isbns[0]||'',isbns,reportedIsbns:isbns,isPhysical:Boolean(format&&!/(?:e-?book|online|electronic)/i.test(format)),metadataProvider:'openlibrary',metadataSource:'Open Library',metadataConfidence:'High',metadataMatchMethod:'Exact ISBN via Open Library direct edition'});
}
async function openLibraryEditionAuthors(edition,options={}){
  const entries=Array.isArray(edition?.authors)?edition.authors.slice(0,3):[];
  const keys=uniq(entries.map(entry=>entry?.author?.key||entry?.key||'')).filter(key=>/^\/authors\/OL[0-9A-Z]+A$/.test(key));
  const names=entries.map(entry=>String(entry?.name||entry?.author?.name||'').trim()).filter(Boolean);
  if(names.length||!keys.length)return names.join(', ');
  const fetchImpl=options.fetchImpl||fetch;
  const resolved=await Promise.all(keys.map(async key=>{
    try{const response=await fetchMetadataResponse(`https://openlibrary.org${key}.json`,fetchImpl,options.timeoutMs||5000,{retry:false});if(!response.ok)return'';const author=await response.json();return String(author?.name||'').trim();}catch{return'';}
  }));
  return uniq(resolved).join(', ');
}
async function lookupOpenLibraryDirectISBNCandidates(isbn,options={}){
  const requested=toISBN13(normalizeISBN(isbn));if(!requested||!isValidISBN(requested))return[];
  const fetchImpl=options.fetchImpl||fetch;
  for(const variant of isbnVariants(requested)){
    const strategy='direct-edition-isbn-'+variant;
    try{
      const response=await fetchMetadataResponse(`https://openlibrary.org/isbn/${variant}.json`,fetchImpl,options.timeoutMs||5000,{retry:false});
      if(!response.ok){recordMetadataTrace(options,'openlibrary',strategy,response.status===404?'NO_RECORD':metadataFailureState(response),{httpStatus:response.status});continue;}
      const edition=await response.json(),candidate=openLibraryEditionToMeta(edition);
      const exact=candidate&&hasOnlyRequestedSourceIsbns(candidate,requested)?selectBestExactMetadataCandidates([candidate],requested):[];
      if(exact.length){exact[0].authors=await openLibraryEditionAuthors(edition,options);recordMetadataTrace(options,'openlibrary',strategy,'FOUND');return selectBestExactMetadataCandidates(exact,requested);}
      recordMetadataTrace(options,'openlibrary',strategy,candidate?(providerReportedIsbns(candidate).length?'WRONG_ISBN':'UNCONFIRMED'):'NO_RECORD');
    }catch(error){recordMetadataTrace(options,'openlibrary',strategy,metadataFailureState(error));}
  }
  return[];
}
async function lookupGoogleBooksDirectISBNCandidates(isbn,options={}){
  const requested=toISBN13(normalizeISBN(isbn));if(!requested||!isValidISBN(requested))return[];
  const fetchImpl=options.fetchImpl||fetch;
  for(const variant of isbnVariants(requested)){
    const strategy='direct-isbn-'+variant;
    try{
      const url=new URL('https://www.googleapis.com/books/v1/volumes');url.search=new URLSearchParams({q:`isbn:${variant}`,maxResults:'5'});
      const response=await fetchMetadataResponse(url.toString(),fetchImpl,options.timeoutMs||5000,{retry:false});
      if(!response.ok){recordMetadataTrace(options,'google',strategy,metadataFailureState(response),{httpStatus:response.status});continue;}
      const data=await response.json(),candidates=(Array.isArray(data.items)?data.items:[]).map(item=>{
        const volume=item?.volumeInfo,meta=googleVolumeToMeta(volume);
        if(!meta)return null;
        const sourceIsbns=uniq((Array.isArray(volume?.industryIdentifiers)?volume.industryIdentifiers:[]).filter(entry=>['ISBN_10','ISBN_13'].includes(entry?.type)).map(entry=>normalizeISBN(entry.identifier)).filter(value=>value&&isValidISBN(value)).map(toISBN13));
        return{...meta,isbn:sourceIsbns[0]||'',isbns:sourceIsbns,reportedIsbns:sourceIsbns};
      }).filter(Boolean);
      const exact=selectBestExactMetadataCandidates(candidates.filter(candidate=>hasOnlyRequestedSourceIsbns(candidate,requested)),requested);
      recordMetadataTrace(options,'google',strategy,exact.length?'FOUND':(candidates.length?'WRONG_ISBN':'NO_RECORD'));
      if(exact.length)return exact;
    }catch(error){recordMetadataTrace(options,'google',strategy,metadataFailureState(error));}
  }
  return[];
}
...
async function orchestrateISBNMetadata(isbn,providers={}){
  ...
  const options={trace:providers.trace,retryDelayMs:providers.retryDelayMs,timeoutMs:providers.timeoutMs,fetchImpl:providers.fetchImpl};
  const candidates=[];
  ...
  const mainCandidates=reconcileExactMetadataCandidates(candidates,requested);
  if(mainCandidates.some(candidate=>candidate.exactIsbn&&hasUsableMetadata(candidate)))return mainCandidates;
  const openLibraryDirect=providers.openLibraryDirect||lookupOpenLibraryDirectISBNCandidates;
  const googleDirect=providers.googleDirect||lookupGoogleBooksDirectISBNCandidates;
  for(const [lookup,name] of [[openLibraryDirect,'openlibrary'],[googleDirect,'google']]){
    const result=await safeLookup(lookup,name);
    const exact=selectBestExactMetadataCandidates(result.candidates,requested).filter(candidate=>candidate.metadataMatchEvidence==='IDENTIFIER_CONFIRMED'&&hasOnlyRequestedSourceIsbns(candidate,requested));
    if(exact.length)return exact;
  }
...
async function lookupISBNMetadataCandidates(isbn,book,options={}){return (await lookupISBNMetadataBaseCandidates(isbn,options)).map(candidate=>enrichCandidateMeta(book||{isbn},candidate,{metadataSource:candidate.metadataSource,metadataMatchMethod:candidate.metadataMatchMethod}));}
...
function isTrustedExactMetadataCandidate(candidate,isbn){
  const requested=toISBN13(normalizeISBN(isbn)),reported=providerReportedIsbns(candidate);
  if(!requested||!isValidISBN(requested)||!candidate?.exactIsbn||!hasUsableMetadata(candidate))return false;
  if(candidate.metadataMatchEvidence==='IDENTIFIER_CONFIRMED')return reported.includes(requested);
  const supportingData=Boolean(String(candidate.authors||'').trim()||String(candidate.publisher||'').trim()||String(candidate.year||'').trim());
  return candidate.metadataMatchEvidence==='PROVIDER_EXACT_QUERY'&&['dnb','loc'].includes(candidate.metadataProvider)&&candidate.exactQueryIsbn===requested&&candidate.exactQueryRecordCount===1&&!reported.length&&supportingData;
}
function metadataRetryIsIncomplete(trace){return(trace||[]).some(entry=>['RATE_LIMITED','TEMPORARY_ERROR','TIMEOUT','HTTP_ERROR','PARSE_ERROR'].includes(entry.state));}
function metadataRetryFeedback(isbn,trace,coverExists){
  if(metadataRetryIsIncomplete(trace))return{state:'retryable',message:'Metadata lookup incomplete. Try again later.'};
  if(coverExists===true)return{state:'not-found',message:'No trusted metadata was returned, but a cover exists for this ISBN. Try Identify for manual search.'};
  return{state:'not-found',message:`No trusted metadata was returned for ISBN ${isbn}. Try Identify for manual search.`};
}
function probeOpenLibraryCover(isbn,options={}){
  const requested=toISBN13(normalizeISBN(isbn));if(!requested||!isValidISBN(requested))return Promise.resolve(null);
  const ImageCtor=options.ImageCtor||(typeof Image==='function'?Image:null);if(!ImageCtor)return Promise.resolve(null);
  return new Promise(resolve=>{
    let image;try{image=new ImageCtor();}catch{resolve(null);return;}let settled=false;
    const finish=value=>{if(settled)return;settled=true;clearTimeout(timer);image.onload=null;image.onerror=null;resolve(value);};
    const timer=setTimeout(()=>finish(null),options.timeoutMs||2500);
    image.onload=()=>finish(true);image.onerror=()=>finish(false);
    try{image.src=`https://covers.openlibrary.org/b/isbn/${requested}-S.jpg?default=false`;}catch{finish(null);}
  });
}
function applyExactMetadataToUnidentifiedCopies(books,copyId,candidate){
  const source=Array.isArray(books)?books:[],selected=source.find(book=>(book.copyId||book.id)===copyId);
  if(!selected||!needsIdentification(selected))return{books,updated:0};
  const requested=toISBN13(normalizeISBN(selected.isbn));if(!requested||!isValidISBN(requested))return{books,updated:0};
  if(!isTrustedExactMetadataCandidate(candidate,requested))return{books,updated:0};
  const unresolved=source.map((book,index)=>({book,index})).filter(({book})=>needsIdentification(book)&&isValidISBN(book.isbn)&&toISBN13(book.isbn)===requested);
  if(!unresolved.some(({book})=>(book.copyId||book.id)===(selected.copyId||selected.id)))return{books,updated:0};
  const identifiedPeer=source.find(book=>book.id!==selected.id&&!needsIdentification(book)&&!isUnidentifiedTitle(book.title)&&isValidISBN(book.isbn)&&toISBN13(book.isbn)===requested);
  const next=source.slice();let targetIdentity=identifiedPeer?{workId:identifiedPeer.workId||v3WorkId(identifiedPeer),editionId:identifiedPeer.editionId||v3EditionId(identifiedPeer)}:null;
  for(const {book,index} of unresolved){
    const updated=applySelectedMetadataCandidate(book,candidate,next);
    if(!targetIdentity){const seed={...updated,workId:'',editionId:''};const workId=v3WorkId(seed);targetIdentity={workId,editionId:v3EditionId({...seed,workId})};}
    next[index]=normalizeBook({...updated,...targetIdentity,needsIdentification:false,tags:uniq([...(book.tags||[]),...(candidate.tags||[])])});
  }
  return{books:next,updated:unresolved.length,title:candidate.title};
}
function unresolvedValidISBNGroups(books=[]){
  const groups=new Map();
  for(const book of Array.isArray(books)?books:[]){
    if(!needsIdentification(book))continue;
    const raw=normalizeISBN(book.isbn);if(!raw||!isValidISBN(raw))continue;
    const isbn=toISBN13(raw);if(!isbn||!isValidISBN(isbn))continue;
    if(!groups.has(isbn))groups.set(isbn,[]);groups.get(isbn).push(book);
  }
  return Array.from(groups,([isbn,copies])=>({isbn,copies}));
}
async function runSequentialUnidentifiedIsbnRetries({books=[],retryOne,onProgress,yieldControl=()=>new Promise(resolve=>setTimeout(resolve,0))}={}){
  const groups=unresolvedValidISBNGroups(books),workingBooks=Array.isArray(books)?books.slice():[];
  const summary={total:groups.length,identified:0,unresolved:0,temporaryFailures:0,skipped:0,updatedCopies:0,results:[]};
  const progressCounts=()=>({identified:summary.identified,unresolved:summary.unresolved,temporaryFailures:summary.temporaryFailures,updatedCopies:summary.updatedCopies});
  for(let index=0;index<groups.length;index++){
    const {isbn}=groups[index],stillUnresolved=workingBooks.some(book=>needsIdentification(book)&&isValidISBN(book.isbn)&&toISBN13(normalizeISBN(book.isbn))===isbn);
    if(!stillUnresolved){summary.skipped++;summary.results.push({isbn,state:'skipped',updatedCopies:0});onProgress?.({phase:'complete',index:index+1,total:groups.length,isbn,...progressCounts()});await yieldControl();continue;}
    onProgress?.({phase:'checking',index:index+1,total:groups.length,isbn,...progressCounts()});
    let outcome;
    try{outcome=await retryOne({books:workingBooks,isbn,trace:[]});}catch(error){outcome={state:'temporary-failure',error,updatedCopies:0};}
    if(Array.isArray(outcome?.books))workingBooks.splice(0,workingBooks.length,...outcome.books);
    const state=outcome?.state==='identified'&&Number(outcome.updatedCopies)>0?'identified':outcome?.state==='temporary-failure'?'temporary-failure':outcome?.state==='skipped'?'skipped':'unresolved';
    const updatedCopies=state==='identified'?Number(outcome.updatedCopies):0;
    summary[state==='temporary-failure'?'temporaryFailures':state==='identified'?'identified':state]++;
    summary.updatedCopies+=updatedCopies;
    summary.results.push({isbn,state,title:outcome?.title||'',candidate:outcome?.candidate||null,updatedCopies,trace:Array.isArray(outcome?.trace)?outcome.trace:[]});
    onProgress?.({phase:'complete',index:index+1,total:groups.length,isbn,...progressCounts()});
    await yieldControl();
  }
  return{books:workingBooks,...summary};
}
function formatRetryAllSummary(summary){
  const identified=Number(summary?.identified)||0,unresolved=Number(summary?.unresolved)||0,temporary=Number(summary?.temporaryFailures)||0,updated=Number(summary?.updatedCopies)||0;
  if(!identified)return`No additional books were identified. ${unresolved} ISBN${unresolved===1?'':'s'} remain unresolved${temporary?` · ${temporary} temporary failures`:''}.`;
  return`Retry complete · ${identified} ISBN${identified===1?'':'s'} identified · ${unresolved} still unresolved${temporary?` · ${temporary} temporary failures`:''} · ${updated} physical cop${updated===1?'y':'ies'} updated`;
}
async function retryUnidentifiedIsbnAgainstCatalog({books=[],isbn,trace=[],checkCover=false}={}){
  const requested=toISBN13(normalizeISBN(isbn));
  if(!requested||!isValidISBN(requested))return{books,state:'skipped',updatedCopies:0,trace};
  const source=Array.isArray(books)?books:[];
  const current=source.find(book=>needsIdentification(book)&&isValidISBN(book.isbn)&&toISBN13(normalizeISBN(book.isbn))===requested);
  if(!current)return{books,state:'skipped',updatedCopies:0,trace};
  const candidates=await lookupISBNMetadataCandidates(requested,current,{trace});
  const candidate=candidates.find(result=>isTrustedExactMetadataCandidate(result,requested));
  if(!candidate){
    const state=metadataRetryIsIncomplete(trace)?'temporary-failure':'unresolved';
    const coverExists=checkCover&&state==='unresolved'?await probeOpenLibraryCover(requested):null;
    return{books,state,title:'',updatedCopies:0,coverExists,feedback:checkCover?metadataRetryFeedback(requested,trace,coverExists):null,trace};
  }
  const result=applyExactMetadataToUnidentifiedCopies(source,current.copyId||current.id,candidate);
  if(!result.updated)return{books,state:'skipped',title:candidate.title,updatedCopies:0,trace};
  return{books:result.books,state:'identified',title:candidate.title,candidate,updatedCopies:result.updated,trace};
}
...
function CameraBarcodeScanner({active,onDetected,onStop,minimal=false,onInvalidDetected,onUnavailable,mode='add'}){
  ...
  useEffect(()=>{
    ...
    const handleFrame=value=>{
      ...
      setState({status:valid?'found':'scanning',message:valid?`Scanned ${toISBN13(frame.code)}. ${['find','move','audit'].includes(mode)?'Checking…':'Adding…'}`:'That does not look like a book barcode.'});
      if(!valid)onInvalidDetected?.(frame.code);
      Promise.resolve(onDetected?.(frame.code)).catch(()=>{const message=['find','move','audit'].includes(mode)?'The book could not be checked. Enter the ISBN below.':'The book could not be added. Use the Scan Desk instead.';setState({status:'error',message});onUnavailable?.(message);}).finally(()=>{if(!cancelled)setState({status:'scanning',message:'Point the camera at the ISBN barcode.'});});
    };
  ...
  const label=state.status==='scanning'?'Scanning':state.status==='found'?(['find','move','audit'].includes(mode)?'Checking book':'Adding book'):state.status==='error'?'Camera needs help':'Starting camera';
...
function BookcaseView({books,collections,activeCollection,search,onEdit,onDropBook,onRetryMetadata,retryingIsbns,dragging,setDragging,onAddCollection,setActiveCollection,onRemoveFromCollection,onRenameCollection,onDeleteCollection,onOpenSettings}){
...
function BookcaseBookCard({book,collections,onEdit,onDropBook,onRetryMetadata,retryingIsbns,setDragging,unshelved,name,mode='move',onRemoveFromCollection,compact=false}){
...
function IntakeQueueView({books,collections,onEdit,onIdentify,onRetryMetadata,onRetryAllISBNs,retryingIsbns,bulkRetryProgress,bulkRetrySummary,onBulkIntakeUpdate,onOpenSort,setActiveView,setFilters,onOpenAddBooks,focusScope,onFocusHandled}){
  ...
  const retryISBNCount=unresolvedValidISBNGroups(identification).length;
  ...
  return <div className="workspace intake-shell organize-shell"><div><div className="overline">Organize</div><h1 className="title">To organize</h1><p className="subtitle">A calm place to identify books, add them to collections, and resolve the few details that still need attention.</p></div>
    {allClear?<section className="organize-complete"><div className="organize-complete-mark"><Icon name="check" size={18}/></div><div><h2>Everything is organized.</h2><p>Your books are identified, shelved, and up to date.</p></div><div className="row-actions"><button className="btn" onClick={onOpenAddBooks}>Add books</button><button className="ghost" onClick={()=>setActiveView('library')}>Open library</button></div></section>:<section className="organize-flow" aria-label="Organization tasks">
      <div className={identification.length?'organize-flow-card':'organize-flow-empty'}><div className="organize-flow-count"><strong>{identification.length}</strong><span>{identification.length===1?'book':'books'}</span></div><div><h2>Needs identification</h2><p>{identification.length?'Books that need a title, author, or metadata check.':'No books need identification. Scanned books with unresolved details will appear here.'}</p>{identification.length>0&&!retryISBNCount&&<p className="small">No unidentified books with valid ISBNs to retry. Manual Identify remains available.</p>}</div>{identification.length>0&&<button className="btn" onClick={()=>openList('identification')}>Review books</button>}</div>
    ...
    {listScope&&<section className="organize-list-section"><div className="organize-list-head"><div><h2>{scopeLabels[listScope]||'Books'}</h2><p className="small">{rows.length} book{rows.length===1?'':'s'} shown · {selected.length} selected</p>{listScope==='identification'&&retryISBNCount===0&&<p className="small" role="status">No unidentified books with valid ISBNs to retry. Manual Identify remains available.</p>}{listScope==='identification'&&bulkRetryProgress&&<p className="small" role="status">{bulkRetryProgress.phase==='waiting'?'Waiting for current ISBN retry…':`Retrying ${bulkRetryProgress.index} of ${bulkRetryProgress.total}…`} {bulkRetryProgress.identified} identified · {bulkRetryProgress.unresolved} unresolved{bulkRetryProgress.temporaryFailures?` · ${bulkRetryProgress.temporaryFailures} temporary failures`:''} · {bulkRetryProgress.updatedCopies} physical copies updated</p>}{listScope==='identification'&&!bulkRetryProgress&&bulkRetrySummary&&<p className="small" role="status">Last retry: {bulkRetrySummary.identified} identified · {bulkRetrySummary.unresolved} unresolved · {bulkRetrySummary.temporaryFailures} temporary failures · {bulkRetrySummary.updatedCopies} physical copies updated</p>}</div><div className="row-actions">{listScope==='identification'&&<button className="ghost" disabled={!retryISBNCount||Boolean(bulkRetryProgress)} onClick={onRetryAllISBNs}>{bulkRetryProgress?`Retrying ${bulkRetryProgress.index||0} of ${bulkRetryProgress.total}…`:`Retry all ISBNs${retryISBNCount?` (${retryISBNCount})`:''}`}</button>}<button className="ghost" onClick={toggleAll} disabled={!rows.length}>{rows.length&&selected.length===rows.length?'Clear selection':'Select all shown'}</button><button className="ghost" onClick={()=>{setListScope('');setSelected([])}}>Done</button></div></div><div className="intake-list">{rows.length?rows.map(book=><div key={book.id} className="intake-row"><input type="checkbox" checked={selected.includes(book.id)} onChange={()=>toggle(book.id)} aria-label={'Select '+(book.title||'book')}/><div className="cover-mini">{book.cover?<img loading="lazy" src={book.cover} alt="" style={{width:'100%',height:'100%',objectFit:'cover',borderRadius:3}}/>:<Icon name="book" size={18}/>}<ReadBadge book={book}/></div><button className="intake-row-main" onClick={()=>onEdit(book)}><b className="truncate">{book.title||'Untitled book'}</b><span className="truncate">{book.authors||'Unknown author'} · {collectionNames(book).join(', ')||'Unshelved'}{locationText(book)?' · '+locationText(book):''}</span></button><div className="clean-row-meta">{needsIdentification(book)&&<><button className="ghost identify-row-action" disabled={!isValidISBN(book.isbn)||retryingIsbns?.has(toISBN13(book.isbn))} onClick={()=>onRetryMetadata?.(book.id)} aria-label={'Retry metadata for '+(book.title||'this book')}>{retryingIsbns?.has(toISBN13(book.isbn))?'Looking up…':'Retry metadata'}</button><button className="btn identify-row-action" onClick={()=>onIdentify(book)} aria-label={'Identify '+(book.title||'this book')}>Identify</button></>}<span className="chip gray">{statusLabel(book.status)}</span>{needsIdentification(book)&&<span className="chip warn">Needs identification</span>}{!needsIdentification(book)&&needsReview(book,safe)&&<span className="chip warn">{reasonFor(book)}</span>}</div></div>):<div className="clean-empty">No matching books. Try changing or clearing your search.</div>}</div></section>}
...
function LocationsView({books,setActiveView,setFilters,onRenameLocation,onMergeLocation,onClearLocation,onUpdateCopyLocations,onEdit,onAuditShelf}){
  ...
  const selectedShelfRooms=selectedNode?.field==='shelf'?[...new Set(selectedNode.books.map(book=>normalizeLocation(book.location).room))].sort((a,b)=>a.localeCompare(b)):[];
  return <div className="workspace"><div className="overline">Locations</div><h1 className="title">Physical inventory</h1><p className="subtitle">Find, assign, move, audit, and browse exact physical copies by where they are stored.</p><button className="btn" onClick={()=>onAuditShelf(blankLocation())}>Audit a shelf</button>
...
function ShelfAuditView({catalog,books,initialTarget,onMoveCopy,onEditCopy,onActivityChange,onBack}){
  const [target,setTarget]=useState(()=>normalizeLocation(initialTarget));
  const [session,setSession]=useState(null),sessionRef=useRef(null);
  const [input,setInput]=useState(''),[cameraOn,setCameraOn]=useState(false),[cameraIssue,setCameraIssue]=useState('');
  const [selectedCandidates,setSelectedCandidates]=useState({}),[corrected,setCorrected]=useState({}),[correctionError,setCorrectionError]=useState('');
  const inputRef=useRef(null);
  const index=useMemo(()=>buildBookcaseNavigationIndex(catalog),[catalog]);
  const summary=session?auditSummary(session):null;
  const last=session?.lastFeedback;
  const shelves=index.bookcases.flatMap(bookcase=>bookcase.shelves.map(shelf=>({bookcase:bookcase.label,shelf:shelf.label,room:bookcase.room,count:shelf.copies.length,key:bookcase.id+'|'+shelf.label})));
  const setAudit=next=>{sessionRef.current=next;setSession(next);onActivityChange(Boolean(next?.events.length&&!next.finished));};
  useEffect(()=>()=>onActivityChange(false),[]);
  useEffect(()=>{const leave=event=>{if(!sessionRef.current?.events.length||sessionRef.current.finished)return;event.preventDefault();event.returnValue='';};window.addEventListener('beforeunload',leave);return()=>window.removeEventListener('beforeunload',leave);},[]);
  useEffect(()=>{if(session&&!session.finished)inputRef.current?.focus();},[session?.finished]);
  function start(){
    if(!hasMoveDestination(target))return;
    setAudit(createShelfAuditState(catalog,normalizeLocation(target)));
    setCameraIssue('');setCorrected({});setSelectedCandidates({});setCorrectionError('');
  }
  function scan(raw,source='manual'){
    const current=sessionRef.current;if(!current||current.finished)return;
    const next=applyShelfAuditScan(current,raw,{source,now:Date.now()});
    if(next!==current)setAudit(next);
    if(source!=='camera')inputRef.current?.focus();
  }
  function submit(event){event.preventDefault();if(!input.trim())return;scan(input,'manual');setInput('');}
  function undo(){const next=undoShelfAuditScan(sessionRef.current);if(next!==sessionRef.current)setAudit(next);inputRef.current?.focus();}
  function finish(){setCameraOn(false);setAudit(finishShelfAudit(sessionRef.current));}
  function toggleManual(copyId){setAudit(markShelfAuditPresent(sessionRef.current,copyId));}
  function correct(event,copy){
    if(!session?.finished||!copy)return;
    const current=locationText({location:copy.location})||'Location not assigned';
    const destination=locationText({location:session.target});
    if(!window.confirm(`Move catalog location for physical Copy ${copy.copyId} from ${current} to ${destination}? A safety snapshot will be created.`))return;
    const result=onMoveCopy(copy.copyId,copy.location,session.target);
    if(result?.error){setCorrectionError(result.error);return;}
    setCorrectionError('');setCorrected(previous=>({...previous,[event.index]:true}));
  }
  const bookById=copyId=>books.find(book=>String(book.copyId||book.id)===copyId);
  const titleOf=copy=>copy.title||copy.isbn||'Untitled book';
  const copyLine=copy=><><b>{titleOf(copy)}</b><span className="small">{copy.authors||'Unknown author'} · {copy.isbn||'No valid ISBN'} · Copy {copy.copyId}</span><span className="small">{locationText({location:copy.location})}</span></>;
  return <div className="workspace shelf-audit"><div className="shelf-audit-head"><div><div className="overline">Physical inventory</div><h1 className="title">Audit a shelf</h1><p className="subtitle">Scan the books actually present. Catalog data stays unchanged until you explicitly confirm a correction.</p></div><button className="ghost" onClick={onBack}>Back to Physical Inventory</button></div>
    {!session?<section className="panel panel-pad shelf-audit-setup"><h2>Choose the exact shelf</h2><p className="small">Bookcase and Shelf are required. Room distinguishes otherwise identical Bookcases; Box and Position do not affect shelf membership.</p><div className="shelf-audit-fields"><label><span className="label">Bookcase *</span><input className="field" value={target.bookcase} onChange={event=>setTarget(current=>({...current,bookcase:event.target.value,shelf:''}))}/></label><label><span className="label">Shelf *</span><input className="field" value={target.shelf} onChange={event=>setTarget(current=>({...current,shelf:event.target.value}))}/></label><label><span className="label">Room (optional)</span><input className="field" value={target.room} onChange={event=>setTarget(current=>({...current,room:event.target.value}))}/></label></div>{shelves.length>0&&<div className="shelf-audit-existing"><div className="label">Catalog shelves</div><div className="shelf-audit-shelf-options">{shelves.map(shelf=><button className="ghost" key={shelf.key} type="button" onClick={()=>setTarget({...blankLocation(),bookcase:shelf.bookcase,shelf:shelf.shelf,room:shelf.room})}>{shelf.bookcase} · Shelf {shelf.shelf}<small>{shelf.room?' · Room: '+shelf.room:' · No Room'} · {shelf.count} {shelf.count===1?'Copy':'Copies'}</small></button>)}</div></div>}<button className="btn shelf-audit-large" onClick={start} disabled={!hasMoveDestination(target)}>Start shelf audit</button></section>:<>
      <section className="shelf-audit-target"><div className="overline">Auditing this exact shelf</div><strong>{session.target.bookcase}</strong><strong>Shelf {session.target.shelf}</strong><span>{session.target.room?'Room: '+session.target.room:'No Room recorded'}</span></section>
      {!session.finished?<><div className="shelf-audit-stats" aria-label="Live audit counters"><div><strong>{summary.expected}</strong><span>Expected</span></div><div><strong>{summary.scanned}</strong><span>Scanned</span></div><div><strong>{summary.matched}</strong><span>Matched</span></div><div><strong>{summary.unexpected}</strong><span>Unexpected</span></div></div>
        <section className="panel panel-pad shelf-audit-scan"><form onSubmit={submit}><label className="label" htmlFor="shelf-audit-isbn">ISBN · manual or USB/Bluetooth scanner</label><div className="find-isbn-row"><input id="shelf-audit-isbn" ref={inputRef} className="field mono" inputMode="numeric" autoComplete="off" value={input} onChange={event=>setInput(event.target.value)} placeholder="Scan or enter the next ISBN…"/><button className="btn" type="submit">Check book</button></div></form><button className="ghost shelf-audit-camera-toggle" onClick={()=>{setCameraIssue('');setCameraOn(on=>!on)}}>{cameraOn?'Stop camera':'Start camera scan'}</button>{cameraOn&&<CameraBarcodeScanner active={cameraOn} mode="audit" onDetected={code=>scan(code,'camera')} onStop={()=>setCameraOn(false)} onUnavailable={setCameraIssue}/>}{cameraIssue&&<p className="small" role="alert">{cameraIssue} Manual or hardware ISBN entry remains available.</p>}
          {last&&<div className={'shelf-audit-last '+last.kind} role="status" aria-live="polite"><strong>{last.kind==='expected'?'✓ ':last.kind==='invalid'?'': '! '}{last.label}</strong>{last.title&&<span>{last.title}</span>}{last.kind==='owned-elsewhere'&&<small>Catalog: {last.candidates.map(copy=>locationText({location:copy.location})||'Location not assigned').join(' / ')}</small>}</div>}
          <div className="shelf-audit-actions"><button className="ghost shelf-audit-large" onClick={undo} disabled={!session.events.length}>Undo last scan</button><button className="btn shelf-audit-large" onClick={finish}>Finish audit</button></div></section>
        {summary.manualNeeded.length>0&&<section className="panel panel-pad shelf-audit-section"><h2>Manual check needed · {summary.manualNeeded.length}</h2><p className="small">These expected Copies have no valid ISBN and cannot be matched by barcode.</p>{summary.manualNeeded.map(copy=><div className="shelf-audit-row" key={copy.copyId}>{copyLine(copy)}<button className="ghost" onClick={()=>toggleManual(copy.copyId)}>Mark present</button></div>)}</section>}</>:<>
        <section className="panel panel-pad shelf-audit-complete"><h2>Shelf audit complete</h2><div className="shelf-audit-stats"><div><strong>{summary.accounted}</strong><span>Correctly accounted for</span></div><div><strong>{summary.missing.reduce((total,group)=>total+group.count,0)}</strong><span>Missing from shelf</span></div><div><strong>{summary.events.filter(event=>['owned-elsewhere','extra'].includes(event.kind)).length}</strong><span>Unexpected books</span></div><div><strong>{summary.notInLibrary}</strong><span>Not in library</span></div><div><strong>{summary.manualNeeded.length}</strong><span>Manual checks needed</span></div></div><p className="small">ISBN scans establish occurrence counts. Matching scans cannot prove which identical physical Copy was handled.</p></section>
        <section className="panel panel-pad shelf-audit-section"><h2>Correctly accounted for · {summary.accounted}</h2><p className="small">{summary.matchedByISBN} physical {summary.matchedByISBN===1?'Copy was':'Copies were'} matched automatically by ISBN.</p>{summary.correctlyShelved.length?summary.correctlyShelved.map(group=><div className="shelf-audit-row" key={group.isbn}><b>{titleOf(group.copies[0])}</b><span className="small">ISBN {group.isbn} · {group.copies.length} expected {group.copies.length===1?'Copy':'Copies'} · {group.count} {group.count===1?'scan':'scans'} matched</span></div>):<p className="small">No expected ISBN occurrences were matched automatically.</p>}{summary.manualPresent.length>0&&<p className="small">{summary.manualPresent.length} no-ISBN {summary.manualPresent.length===1?'Copy was':'Copies were'} marked present manually; these are not ISBN matches.</p>}</section>
        <section className="panel panel-pad shelf-audit-section"><h2>Missing from shelf · {summary.missing.reduce((total,group)=>total+group.count,0)}</h2>{summary.missing.length?summary.missing.map(group=><div className="shelf-audit-row" key={group.isbn}><b>{titleOf(group.copies[0])}</b><span className="small">{group.copies[0].authors||'Unknown author'} · ISBN {group.isbn} · {group.count} expected {group.count===1?'Copy':'Copies'} not scanned</span><span className="small">Catalog: {locationText({location:group.copies[0].location})}</span>{group.copies.length>1&&<span className="small">Possible Copy IDs: {group.copies.map(copy=>copy.copyId).join(', ')}. ISBN alone cannot identify which is missing.</span>}{group.copies.map(copy=><button className="ghost" key={copy.copyId} onClick={()=>onEditCopy(bookById(copy.copyId))}>Open details{group.copies.length>1?' · '+copy.copyId:''}</button>)}</div>):<p className="small">All scannable expected occurrences were accounted for.</p>}</section>
        <section className="panel panel-pad shelf-audit-section"><h2>Unexpected on shelf · {summary.events.filter(event=>['owned-elsewhere','extra'].includes(event.kind)).length}</h2>{summary.events.filter(event=>['owned-elsewhere','extra'].includes(event.kind)).map(event=><div className="shelf-audit-row" key={event.index}><b>{event.title}</b><span className="small">ISBN {event.isbn} · Scanned on: {locationText({location:session.target})}</span>{event.kind==='owned-elsewhere'?<><span className="small">Catalog says: {event.candidates.map(copy=>locationText({location:copy.location})||'Location not assigned').join(' / ')}</span>{corrected[event.index]?<span className="chip good">Catalog updated</span>:event.candidates.length===1?<button className="btn" onClick={()=>correct(event,event.candidates[0])}>Move catalog location here</button>:<><label><span className="label">Choose physical copy</span><select className="select" value={selectedCandidates[event.index]||''} onChange={change=>setSelectedCandidates(previous=>({...previous,[event.index]:change.target.value}))}><option value="">Select exact Copy…</option>{event.candidates.map(copy=><option key={copy.copyId} value={copy.copyId}>{copy.copyId} · {locationText({location:copy.location})||'Location not assigned'}</option>)}</select></label><button className="btn" disabled={!selectedCandidates[event.index]} onClick={()=>correct(event,event.candidates.find(copy=>copy.copyId===selectedCandidates[event.index]))}>Move selected Copy here</button></>}</>:<span className="small">More scans than cataloged physical Copies with this ISBN. Check the book manually.</span>}</div>)}{!summary.events.some(event=>['owned-elsewhere','extra'].includes(event.kind))&&<p className="small">No unexpected owned books were scanned.</p>}{correctionError&&<p className="small bad" role="alert">{correctionError}</p>}</section>
        <section className="panel panel-pad shelf-audit-section"><h2>Not in library · {summary.notInLibrary}</h2>{summary.events.filter(event=>event.kind==='not-owned').map(event=><div className="shelf-audit-row" key={event.index}><b>ISBN {event.isbn}</b><span className="small">No owned Copy matched. Nothing was added.</span></div>)}{!summary.notInLibrary&&<p className="small">No unowned ISBNs were scanned.</p>}</section>
        <section className="panel panel-pad shelf-audit-section"><h2>Manual check needed · {summary.manualNeeded.length}</h2>{summary.manualNeeded.length?summary.manualNeeded.map(copy=><div className="shelf-audit-row" key={copy.copyId}>{copyLine(copy)}<button className="ghost" onClick={()=>toggleManual(copy.copyId)}>Mark present</button></div>):<p className="small">No manual checks remain.</p>}{summary.manualPresent.length>0&&<div className="shelf-audit-manual-present"><h3>Marked present by hand</h3>{summary.manualPresent.map(copy=><div className="shelf-audit-row" key={copy.copyId}>{copyLine(copy)}<button className="ghost" onClick={()=>toggleManual(copy.copyId)}>Undo mark</button></div>)}</div>}</section>
      </>}
    </>}
  </div>;
}
...
function App(){
  const [booksState,setBookViews]=useState([]);const books=booksState;const [collections,setCollections]=useState([]);const [loaded,setLoaded]=useState(false);const [storageMode,setStorageMode]=useState(()=>getPreferredStorageMode());const [storageInfo,setStorageInfo]=useState({usage:0,quota:0});const [activeView,setActiveViewState]=useState('dashboard');const [activeCollection,setActiveCollection]=useState('');const [search,setSearch]=useState('');const [editing,setEditing]=useState(null);const [identifying,setIdentifying]=useState(null);const [dragging,setDragging]=useState(null);const [toasts,setToasts]=useState([]);const [filters,setFilters]=useState(defaultFilters);const [savedViews,setSavedViews]=useState([]);const [settings,setSettings]=useState(DEFAULT_SETTINGS);const [showChangelog,setShowChangelog]=useState(false);const [showCommand,setShowCommand]=useState(false);const [showAddBooks,setShowAddBooks]=useState(false);const [scanFocus,setScanFocus]=useState('');const [organizeFocus,setOrganizeFocus]=useState('');const [pendingImport,setPendingImport]=useState(null);const [pendingMetadata,setPendingMetadata]=useState(null);const [quickEditing,setQuickEditing]=useState(null);const [moveRequest,setMoveRequest]=useState(null);const [scanDestination,setScanDestination]=useState(blankLocation());const [sortDeskQueue,setSortDeskQueue]=useState([]);const [easyCompletionIds,setEasyCompletionIds]=useState([]);const [retryingIsbns,setRetryingIsbns]=useState(()=>new Set());const [bulkRetryProgress,setBulkRetryProgress]=useState(null);const [bulkRetrySummary,setBulkRetrySummary]=useState(null);const [auditInitialTarget,setAuditInitialTarget]=useState(blankLocation());const [migrationBackupStamp,setMigrationBackupStamp]=useState(()=>migrationBackupInfo()?.createdAt||'');const nativeCatalogRef=useRef(blankNativeCatalog());const booksRef=useRef([]);const auditHasScansRef=useRef(false);const retryingIsbnsRef=useRef(new Set());const retryingIsbnCountsRef=useRef(new Map());const singleRetryingIsbnsRef=useRef(new Set());const bulkRetryRunningRef=useRef(false);const storageWarningRef=useRef(false);const backupReminderShownRef=useRef(false);
  function setActiveView(next){const target=typeof next==='function'?next(activeView):next;if(target===activeView)return;if(activeView==='audit'&&auditHasScansRef.current&&!window.confirm('Leave this shelf audit? The scan results for this session will be discarded.'))return;auditHasScansRef.current=false;setActiveViewState(target);}
  ...
  function acquireRetryingIsbn(isbn){const count=(retryingIsbnCountsRef.current.get(isbn)||0)+1;retryingIsbnCountsRef.current.set(isbn,count);retryingIsbnsRef.current.add(isbn);setRetryingIsbns(new Set(retryingIsbnsRef.current));}
  function releaseRetryingIsbn(isbn){const count=retryingIsbnCountsRef.current.get(isbn)||0;if(count<=1){retryingIsbnCountsRef.current.delete(isbn);retryingIsbnsRef.current.delete(isbn);}else retryingIsbnCountsRef.current.set(isbn,count-1);setRetryingIsbns(new Set(retryingIsbnsRef.current));}
...
${recovery}`);
  ...
  function moveExactCopies(copyIds,destination,{silent=false,forceSnapshot=false}={}){
    ...
    if(result.changed){
      if(!silent)makeSnapshot('before-exact-copy-move',forceSnapshot);
  ...
  function moveAuditCopy(copyId,observedLocation,destination){
    if(!hasMoveDestination(destination))return {error:'Choose a Bookcase and Shelf.'};
    const current=nativeCatalogRef.current.copies.find(copy=>copy.id===copyId);
    if(!current)return {error:'This physical Copy is no longer in the catalog. No move was made.'};
    if(!sameLocation(current.location,observedLocation)||normalizeLocation(current.location).position!==normalizeLocation(observedLocation).position)return {error:'This Copy moved since the audit began. Review its current location before correcting it.'};
    return moveExactCopies([copyId],destination,{forceSnapshot:true});
  }
  ...
  function openShelfAudit(target=blankLocation()){setAuditInitialTarget(normalizeLocation(target));setActiveView('audit');}
  ...
  async function retryUnidentifiedMetadata(copyId){
    const current=(booksRef.current||[]).find(book=>(book.copyId||book.id)===copyId);
    const requested=toISBN13(normalizeISBN(current?.isbn));
    if(!current||!needsIdentification(current)||!requested||!isValidISBN(requested))return{state:'invalid'};
    if(bulkRetryRunningRef.current)return{state:'busy'};
    if(retryingIsbnsRef.current.has(requested))return{state:'busy'};
    singleRetryingIsbnsRef.current.add(requested);acquireRetryingIsbn(requested);
    const trace=[];
    try{
      const result=await retryUnidentifiedIsbnAgainstCatalog({books:booksRef.current||[],isbn:requested,trace,checkCover:true});
      if(result.state==='identified'){makeSnapshot('before-retry-unidentified-metadata');setBooks(result.books);toast(result.updatedCopies===1?`Identified: ${result.title}`:`Identified ${result.title} · ${result.updatedCopies} copies updated`);return{...result,updated:result.updatedCopies};}
      if(result.state==='unresolved'||result.state==='temporary-failure'){toast(result.feedback?.message||'Metadata lookup incomplete. Try again later.','error');return{...result,state:result.state==='temporary-failure'?'retryable':'not-found'};}
      return result;
    }catch(error){
      toast('Metadata lookup incomplete. Try again later.','error');
      return{state:'retryable',error};
    }finally{
      singleRetryingIsbnsRef.current.delete(requested);releaseRetryingIsbn(requested);
    }
  }
  async function retryAllUnidentifiedMetadata(){
    if(bulkRetryRunningRef.current)return{state:'busy'};
    const initialBooks=booksRef.current||books,groups=unresolvedValidISBNGroups(initialBooks);
    if(!groups.length){toast('No unidentified books with valid ISBNs to retry.','error');return{state:'empty'};}
    bulkRetryRunningRef.current=true;setBulkRetrySummary(null);
    const alreadyRunning=groups.filter(group=>singleRetryingIsbnsRef.current.has(group.isbn)).map(group=>group.isbn);
    groups.forEach(group=>acquireRetryingIsbn(group.isbn));
    setBulkRetryProgress({phase:alreadyRunning.length?'waiting':'checking',index:0,total:groups.length,isbn:'',identified:0,unresolved:0,temporaryFailures:0,updatedCopies:0});
    try{
      while(alreadyRunning.some(isbn=>singleRetryingIsbnsRef.current.has(isbn)))await new Promise(resolve=>setTimeout(resolve,30));
      await new Promise(resolve=>setTimeout(resolve,0));
      makeSnapshot('before-retry-all-unidentified',true);
      const result=await runSequentialUnidentifiedIsbnRetries({
        books:booksRef.current||books,
        retryOne:({books:workingBooks,isbn,trace})=>retryUnidentifiedIsbnAgainstCatalog({books:workingBooks,isbn,trace}),
        onProgress:progress=>setBulkRetryProgress(progress)
      });
      let latestBooks=(booksRef.current||[]).slice(),identified=0,updatedCopies=0;
      for(const item of result.results){
        if(item.state!=='identified'||!item.candidate)continue;
        const current=latestBooks.find(book=>needsIdentification(book)&&isValidISBN(book.isbn)&&toISBN13(normalizeISBN(book.isbn))===item.isbn);
        if(!current)continue;
        const applied=applyExactMetadataToUnidentifiedCopies(latestBooks,current.copyId||current.id,item.candidate);
        if(applied.updated){latestBooks=applied.books;identified++;updatedCopies+=applied.updated;}
      }
      if(updatedCopies)setBooks(latestBooks);
      await new Promise(resolve=>setTimeout(resolve,0));
      const summary={total:result.total,identified,unresolved:result.unresolved,temporaryFailures:result.temporaryFailures,skipped:result.skipped+(result.identified-identified),updatedCopies};
      setBulkRetrySummary(summary);toast(formatRetryAllSummary(summary),summary.identified?'ok':'error');
      return{state:'complete',...summary};
    }catch(error){
      toast('Bulk metadata retry stopped unexpectedly. Any completed identifications were saved.','error');
      return{state:'error',error};
    }finally{
      groups.forEach(group=>releaseRetryingIsbn(group.isbn));bulkRetryRunningRef.current=false;setBulkRetryProgress(null);
    }
  }
