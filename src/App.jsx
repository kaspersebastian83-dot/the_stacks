...
function VisualBookcase({bookcase,room,unassigned,books,onEdit,onMoveRequest,onOpenScan}){
  ...
  return <section className="visual-bookcase" aria-label={'Visual bookcase '+bookcase.label}>
    ...
    <div className="visual-bookcase-frame">{bookcase.shelves.map(shelf=><section className="visual-shelf" key={shelf.label} aria-label={shelf.label}><div className="visual-shelf-head"><h3>{physicalLocationParts({shelf:shelf.label}).primary}</h3><span className="small">{shelf.copies.length} {shelf.copies.length===1?'Copy':'Copies'}</span><button className="mini-action" onClick={()=>onOpenScan({room,bookcase:bookcase.label,shelf:shelf.label,box:'',position:''})}>Scan to this Shelf</button></div><div className="visual-shelf-books">{sortBookcaseShelfCopies(shelf.copies).map(copy=>{const book=booksByCopyId.get(copy.id),label=[book?.title||'Untitled book',book?.authors,physicalLocationParts(copy.location).primary,copy.location?.position?'Position '+copy.location.position:''].filter(Boolean).join(', ');return <article className="visual-copy" key={copy.id}><button className="visual-copy-book" onClick={()=>book&&onEdit(book)} aria-label={'Open '+label} disabled={!book}><span className="visual-copy-cover">{book?.cover?<img loading="lazy" src={book.cover} alt=""/>:<span>{book?.title||'Untitled book'}</span>}</span><b>{book?.title||'Untitled book'}</b>{book?.authors&&<small>{book.authors}</small>}{copy.location?.position&&<small className="visual-copy-position">#{copy.location.position}</small>}</button><button className="mini-action" onClick={()=>onMoveRequest([copy.id])}>Move</button></article>})}</div><div className="visual-shelf-board" aria-hidden="true"/></section>)}</div>
